    ...
    operator fun invoke(category: String): Flow<Resource<Products>> {
        return flow {
            try {
                ...
                val getProductsByCategory = repo.getProductsByCategory(category)
                getProductsByCategory.status?.let { status ->
                    if (status == 400) {
                        emit(
                            Resource.Error(
                                message = getProductsByCategory.message ?: "Unknown error!"
                            )
                        )
                    }
                    emit(Resource.Success(data = getProductsByCategory.toProducts()))
                }
            } catch (e: Exception) {
