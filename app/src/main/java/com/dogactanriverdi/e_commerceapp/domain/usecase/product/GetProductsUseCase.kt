    ...
    operator fun invoke(): Flow<Resource<Products>> {
        return flow {
            try {
                ...
                val getProducts = repo.getProducts()
                getProducts.status?.let { status ->
                    if (status == 400) {
                        emit(Resource.Error(message = getProducts.message ?: "Unknown error!"))
                    }
                    emit(Resource.Success(data = getProducts.toProducts()))
                }
            } catch (e: Exception) {
